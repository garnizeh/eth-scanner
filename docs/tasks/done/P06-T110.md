...
- [x] Benchmark `BenchmarkConstructPrivateKey` measures key construction speed
- [x] Benchmark `BenchmarkDeriveEthereumAddress` measures address derivation speed
- [x] Benchmark `BenchmarkScanRange` measures single-threaded scanning throughput
- [x] Benchmark `BenchmarkScanRangeParallel` measures multi-threaded scanning throughput
- [x] Benchmarks report allocations per operation (`-benchmem`)
- [x] README or docs document expected throughput on reference hardware
- [x] CI runs benchmarks and checks for regressions (optional)
