...
- [x] Directory `go/internal/` exists
- [x] Directory `go/internal/server/` exists (API handlers, routes, middleware)
- [x] Directory `go/internal/database/` exists (SQLite connection, sqlc generated code)
- [x] Directory `go/internal/jobs/` exists (Job manager, batch allocation logic)
- [x] Directory `go/internal/worker/` exists (PC worker implementation)
- [x] Directory `go/internal/config/` exists (Configuration management)
- [x] All directories are committed to repository
...
- Additional subdirectories can be added later as needed (e.g., `internal/server/handlers/`, `internal/server/middleware/`)
