...
- [x] Test environment uses a clean temporary database.
- [x] Verify `POST /api/v1/jobs/lease` works with a real DB.
- [x] Verify `PATCH /api/v1/jobs/{id}/checkpoint` updates the DB correctly.
- [x] Verify `POST /api/v1/jobs/{id}/complete` updates the DB correctly.
- [x] Verify `POST /api/v1/results` populates any results or stats as expected.
