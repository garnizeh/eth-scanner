...
- [x] Core 0 task detects when a job is needed (e.g., startup or previous job complete).
- [x] Core 0 successfully calls `lease_job` via `api_client`.
- [x] Core 0 updates the global `job_info_t` structure.
- [x] Core 0 uses `xTaskNotify` (or similar) to wake up or signal the Core 1 task.
- [x] Handle "No Content (204)" responses with appropriate backoff.
