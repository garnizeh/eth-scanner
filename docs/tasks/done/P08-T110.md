...
- [x] Use `xTaskCreateStaticPinnedToCore` for the Core 0 task.
- [x] Use `xTaskCreateStaticPinnedToCore` for the Core 1 task.
- [x] Allocate task stacks and `StaticTask_t` buffers in the global scope or as static local variables.
- [x] Ensure the application no longer calls `xTaskCreate` (dynamic version) for worker tasks.
