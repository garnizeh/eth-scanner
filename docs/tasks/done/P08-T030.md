...
- [x] Implement `derive_eth_address(priv_key, address)` function.
- [x] Correctly handle public key prefix removal (strip the first byte `0x04`).
- [x] Correctly take the last 20 bytes of the `keccak256` hash of the public key.
- [x] Validated against known Ethereum private key/address pairs.
