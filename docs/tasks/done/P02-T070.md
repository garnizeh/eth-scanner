...
- [x] Add `github.com/pressly/goose/v3` to go.mod
- [x] Verify goose works with `modernc.org/sqlite` (no CGO)
- [x] Update `ApplySchema()` to use goose.Up() instead of manual execution
- [x] Create `RunMigrations(ctx context.Context, db *sql.DB) error` function
- [x] Add migration status check function
- [x] Update Makefile with `make migrate-up` and `make migrate-status` targets
- [x] Test migration up/down/status commands
