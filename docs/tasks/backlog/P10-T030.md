...
- [ ] Implement a `Hub` struct to manage active WebSocket connections.
- [ ] Create an endpoint `/api/v1/ws` for dashboard clients to connect.
- [ ] Integrate the hub with the `checkpoint` and `complete` handlers to trigger broadcasts.
- [ ] Implement a background "heartbeat" broadcast every 5-10 seconds for general fleet stats.
- [ ] Support HTMX WebSocket extension protocols for easy fragment swapping.
