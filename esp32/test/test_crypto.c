...
void test_crypto_address_comparison(void)
{
    // Mock target address (20 bytes)
    uint8_t target[20] = {
        0x7e, 0x5f, 0x45, 0x52, 0x09, 0x1a, 0x69, 0x12, 0x5d, 0x5d,
        0xfc, 0xb7, 0xb8, 0xc2, 0x65, 0x90, 0x29, 0x39, 0x5b, 0xdf};
    ...
    // Derived address does NOT match target (last byte different)
    uint8_t no_match[20] = {
        0x7e, 0x5f, 0x45, 0x52, 0x09, 0x1a, 0x69, 0x12, 0x5d, 0x5d,
        0xfc, 0xb7, 0xb8, 0xc2, 0x65, 0x90, 0x29, 0x39, 0x5b, 0x01};

    // Binary comparison (memcmp returns 0 on match)
    TEST_ASSERT_EQUAL(0, memcmp(matches, target, 20));
    TEST_ASSERT_NOT_EQUAL(0, memcmp(no_match, target, 20));
}
